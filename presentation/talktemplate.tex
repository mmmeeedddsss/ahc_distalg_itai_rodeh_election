...
\author[Mert Tunc]{Mert Tunc\\\href{mailto:mert.tunc@metu.edu.tr}{mert.tunc@metu.edu.tr}}
%\author[J.\,Doe \& J.\,Doe]
%{%
%  \texorpdfstring{
...
\title[WINS Beamer Template]{Ceng532 Itai-Rodeh Leader Election Algorithm For Rings}
\subtitle[Short SubTitle]{Term project presentation}
%\date{} 
...
%\end{frame}
%
\section{Leader Election In Distributed Systems}
%\begin{frame}
%        \sectionpage
%\end{frame}

\begin{frame}{Leader Election In Distributed Systems}
%\begin{block}{Leader Election In Distributed Systems}
...
%\end{block}
Leader election is a procedure for picking a unique leader process among multiple processes so that,
\begin{itemize}
    \item Leader knows that it has been selected as leader
    \item Other processes know that they have not selected as leader
\end{itemize}

\vspace{1cm}

Leader election is a fundamental requirement for distributed systems since we generally want to ensure that some operations are made by only one process, such as making decisions, transactions, etc.
\note{}
\end{frame}

\section{Itai-Rodeh Leader Election Algorithm}
\begin{frame}
\frametitle{Itai-Rodeh Leader Election Algorithm}
\framesubtitle{}

Itai-Rodeh Leader Election Algorithm describes electing a leader in a system such that:
\begin{itemize}
    \item Communication channels between processes are asynchronous
    \item Processes are anonymous
    \item Communication topology is a ring
\end{itemize}

\vspace{1cm}

The algorithm is a probabilistic algorithm with a termination probability of 1.
